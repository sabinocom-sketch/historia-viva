import{G as W,a as L,d as T,h as A,k as G,l as n,m as H,n as p,p as r,q as M,r as Q,t as U,u as q,v as w,w as j,x as S,y as B,z as K}from"./chunk-6XHDPVNM.js";import{a as V}from"./chunk-UEKKBUIS.js";function _(){let e=n.currentLessonId?S(n.currentLessonId):q(n.currentEra)[0];return e?D(e).mood:""}function ze(){let e=n.currentLessonId?S(n.currentLessonId):q(n.currentEra)[0];if(!e)return"";if(n.currentLessonEntryId!==e.id){n.currentLessonEntryId=e.id,n.currentLessonMode="intro",n.currentLessonStoryBlockIndex=0,n.currentPostStoryStep="summary";let c=ne(e.id);n.currentLessonReflectionText=c.reflectionAnswer||"",n.currentLessonDebateChoice=c.debateChoice||"",n.currentLessonQuizAnswers=c.quizAnswers||{},n.currentPostLessonQuizIndex=0,n.currentLessonQuizChoice=null}let a=n.currentLessonId===e.id,t=D(e),o=Q(L[e.eraKey].curiosities||[],e.index,"Cada vest\xEDgio hist\xF3rico muda quando fazemos uma pergunta melhor."),s=ke(e.title,e.detail,e.category,...e.related||[]),i=n.currentLessonMode==="intro";return`
    <section class="active-lesson-panel lesson-view ${a?"is-active":""}" data-active-lesson="${r(e.id)}" data-era="${r(e.eraKey)}" data-section="${r(e.sectionId||"")}" data-mood="${r(t.mood)}" data-mode="${r(n.currentLessonMode)}" data-theme="${r(e.category)}">
      ...
      </div>`:X(e,{insight:s,curiosity:o})}
    </section>
  `}function X(e,a={}){if(n.currentLessonMode==="story"||n.currentLessonMode==="understand"){let t=v(e.id),o=I(t);return`
    ...
    `}return oe(e,a)}function Y(e,a,t,o={}){let s=e||{id:"story",visualType:"spark",backgroundMood:"cave-dark",text:"Um pequeno momento come\xE7ou a mudar a experi\xEAncia humana."},i=s.eraKey||o.eraKey||"",c=s.sectionId||o.sectionId||"",u=i==="pre-historia"||["paleolitico","mesolitico","revolucao-neolitica"].includes(c),d=u?ee(s,o):"",l=u?Z(s,o):"",m=s.text,h=ae(m),$=a===0?"eager":"lazy",x=a===0?"high":"auto";return`
    <article class="story-block ${u?"storyblock-prehistory cave-firelight":""}" style="--cave-line-count: ${h}" data-story-block="${r(s.id)}" data-story-index="${a%3}" data-visual="${r(s.visualType)}" data-background="${r(s.backgroundMood)}" data-mood="${r(_())}" data-era="${r(i)}" data-section="${r(c)}" data-prehistory-artifact="${r(d)}">
      ...
      <div class="story-block-copy prehistory-stone-panel prehistory-cave-text-area prehistory-narrative-layout cave-pigment-reveal" style="--cave-line-count: ${h}">
        ...
        ${l?`<h3 class="prehistory-narrative-title">${r(l)}</h3>`:""}
  ...
  `}function Z(e={},a={}){return f(a.title||e.title||"Marca na Pedra")}function ee(e={},a={}){let t=p(`${a.title||""} ${e.text||""} ${e.visualType||""}`);return["fogo","chama","fogueira","aquecer","cozinhar","spark","embers"].some(o=>t.includes(o))?"fire":["caca","pesca","animal","rasto","lanca","spear","tracks"].some(o=>t.includes(o))?"hunt":["arte","rupestre","pintura","pigmento","mao","hand"].some(o=>t.includes(o))?"painting":["abrigo","gruta","caverna"].some(o=>t.includes(o))?"shelter":"flint"}function E(e=""){let a=String(e||"").trim();if(!a)return[""];let t=a.split(/(?<=[.!?])\s+|;\s+/).map(o=>o.trim()).filter(Boolean);return t.length?t:[a]}function ae(e=""){return E(e).length}function te(e=""){return`
  ...
  `}function oe(e,a={}){let t=re(e,a),o=se(n.currentLessonMode),s=y.indexOf(o),i=y.length;n.currentPostStoryStep=o;let u={summary:R,reflection:ve,debate:fe,quiz:ye}[o]||R;return`
  ...
  `}function se(e){let a=J[e]||e||n.currentPostStoryStep||"summary";return y.includes(a)?a:"summary"}function re(e,a={}){let t=ie(e,a);return{summary:{summary:t.summary,previous:"story"},reflection:{...t.reflection,previous:"summary"},debate:{...t.debate,previous:"reflection"},quiz:{quiz:t.quiz,previous:"debate",nextLessonId:B(e.eraKey,e.id)?.id||""}}}function ne(e=""){if(typeof localStorage>"u"||!e)return{};try{return JSON.parse(localStorage.getItem("historiaVivaProgress"))?.lessonProgress?.[e]||{}}catch{return{}}}function ie(e,a={}){if(e.postLesson)return e.postLesson;let t=f(e.title),o=w(e.id),s=j(e),i=ce(e,o,s);return{summary:i,reflection:{question:s.reflection||a.insight||`Que mudanca historica te parece mais importante em ${t}?`},debate:de(e,t,s),quiz:le(e,i)}}function ce(e,a=[],t={}){let s=[t.keyTakeaway,...t.evidence||[],...t.archaeology||[],...t.consequences||[],...a.map(i=>i.text),e.detail].filter(Boolean).map(i=>ue(g(i,15),e)).filter(Boolean).filter((i,c,u)=>u.findIndex(d=>p(d)===p(i))===c).slice(0,4);for(;s.length<3;)s.push(g(`${f(e.title)} ajuda a perceber causas, escolhas humanas e consequencias.`,14));return s.slice(0,4)}function ue(e="",a={}){if(!e||C(e)>=9)return e;let t=p(`${a.title||""} ${a.detail||""}`);return t.includes("pedra")||t.includes("ferramenta")?`${e} O gesto humano dava-lhe funcao e sentido.`:t.includes("fogo")?`${e} O uso repetido transformava seguranca, alimento e convivio.`:`${e} O contexto historico ajuda a perceber a mudanca.`}function de(e,a,t={}){let o=g(t.keyTakeaway||e.detail||a,18);return{question:`Como devemos interpretar ${a}: sobretudo como progresso ou como mudanca complexa?`,options:[{id:"optionA",label:"Foi sobretudo progresso",feedback:`Esta posicao pode indicar que ${o.toLowerCase()} abriu possibilidades importantes. Ainda assim, muitos historiadores defendem que ganhos materiais tambem trouxeram novos custos.`},{id:"optionB",label:"Foi uma mudanca complexa",feedback:`Esta posicao sugere que ${o.toLowerCase()} deve ser lido com cautela. Ha debate sobre o peso dos beneficios e dos limites em diferentes grupos sociais.`}]}}function le(e,a=[]){return pe(e,a).slice(0,5)}function pe(e={},a=[]){let t=f(e.title||"esta licao"),o=g(e.detail||a[0]||t,18),s=g(a[0]||e.detail||t,16),i=g(a[1]||e.question||e.detail||t,16),c=g(a[2]||"comparar vestigios, contexto e consequencias",16);return[{question:`Qual e a ideia central da licao "${t}"?`,options:[s,"Foi um acontecimento isolado, sem relacao com o contexto.","Serve apenas para memorizar uma data, sem interpretar causas.","Nao deixou pistas que possam ser estudadas historicamente."],correctIndex:0,explanation:`A resposta correta retoma diretamente o foco da licao "${t}".`},{question:`Que pista ajuda melhor a explicar "${t}"?`,options:[i,"Ignorar os grupos humanos envolvidos.","Separar o acontecimento das suas consequencias.","Escolher uma resposta que podia servir para qualquer licao."],correctIndex:0,explanation:"A melhor pista nasce do conteudo trabalhado nesta licao, nao de outro tema da era."},{question:`Como devemos estudar historicamente "${t}"?`,options:[`Ligando ${c.toLowerCase()} ao contexto da licao.`,"Decorando uma frase sem analisar fontes ou consequencias.","Trocando o tema por outro acontecimento da mesma era.","Assumindo que todos os grupos viveram a mudanca da mesma forma."],correctIndex:0,explanation:`Esta leitura mantem a resposta dentro da licao: ${o}`}].map(me)}function me(e={}){return{question:e.question||"Que ideia fica desta licao?",options:ge(e.options||[]),correctIndex:Number.isInteger(e.correctIndex)?e.correctIndex:Math.min(Math.max(e.answer||0,0),3),explanation:e.explanation||"Esta resposta enquadra melhor o contexto historico da licao."}}function ge(e=[]){let a=["Foi um processo sem contexto.","Aconteceu da mesma forma em todo o lado.","Teve apenas uma causa.","Deve ser lido com fontes e consequencias."],t=e.slice(0,4);return a.forEach(o=>{t.length<4&&!t.includes(o)&&t.push(o)}),t.slice(0,4)}function O(e=""){let a=String(e||"").replace(/\s+/g," ").trim();return a?a.match(/[^.!?…]+(?:[.!?…]+["”’»]?|$)/g)?.map(t=>t.trim()).filter(Boolean)||[a]:[]}function C(e=""){return String(e||"").trim().split(/\s+/).filter(Boolean).length}function g(e="",a=24){let t=O(e);if(!t.length)return"";let o=[],s=0;for(let i of t){let c=C(i);if(o.length&&s+c>a||(o.push(i),s+=c,s>=a))break}return o.join(" ")}function he(e,a){let t=O(e?.text);if(!t.length)return[e];let o=[],s=[],i=0;return t.forEach(c=>{let u=C(c);s.length&&i+u>a&&(o.push(s.join(" ")),s=[],i=0),s.push(c),i+=u}),s.length&&o.push(s.join(" ")),o.map((c,u)=>({...e,id:`${e.id||"story"}-${u+1}`,text:c}))}function v(e){return w(e).flatMap(t=>he(t,55))}function F(e,a,t=""){let o=v(e.id).length,s=Math.max(1,o+y.length),i=Math.min(Math.max(a,0),s-1),c=s<=1?100:Math.round(i/(s-1)*100);return`
  ...
  `}function b(e,a,t){let o=v(t.id).length;return F(t,o+e,"post-story-progress")}function P(e,a="Continuar",t=!1){return`
  ...
  `}function R(e,a,t,o){let s=(e.summary||[]).slice(0,4);return`
    <article class="post-story-screen lesson-summary-screen" data-era="${r(a.eraKey||"")}" data-section="${r(a.sectionId||"")}" data-theme="${r(a.category)}">
      ...
      ${b(t,o,a)}
      ${P(e.previous,"Continuar")}
    </article>
  `}function ve(e,a,t,o){return`
  ...
  `}function fe(e,a,t,o){let s=(e.options||[]).find(i=>i.id===n.currentLessonDebateChoice);return`
  ...
  `}function ye(e,a,t,o){let s=e.quiz||[],i=n.currentLessonQuizAnswers||{},c=s.reduce((z,k,N)=>z+(i[N]===k.correctIndex?1:0),0),u=s.filter((z,k)=>i[k]!==void 0).length,d=s.length>0&&u===s.length,l=d?Math.max(0,s.length-1):Math.min(Math.max(Number(n.currentPostLessonQuizIndex)||0,0),Math.max(0,s.length-1)),m=s[l],h=m&&i[l]!==void 0,$=l<s.length-1,x=!h&&!d?'<button type="button" disabled>Responde para continuar</button>':h&&!d&&$?'<button type="button" data-lesson-action="quiz-next">Proxima pergunta</button>':e.nextLessonId?`<button type="button" data-lesson-action="next-lesson" data-next-lesson="${r(e.nextLessonId)}" ${d?"":"disabled"}>Avancar para a proxima licao</button>`:`<button type="button" data-lesson-action="complete" ${d?"":"disabled"}>Concluir licao</button>`;return`
    <article class="post-story-screen challenge-screen quiz-screen" data-era="${r(a.eraKey||"")}" data-section="${r(a.sectionId||"")}" data-theme="${r(a.category)}" data-quiz-total="${s.length}">
      ...
      <div class="post-story-copy">
        ...
        ${s.length?`<p>Pergunta ${Math.min(l+1,s.length)} de ${s.length}</p>`:""}
      </div>
  ...
  `}function be(e,a,t){let o=t!==void 0,s=o&&t===e.correctIndex;return`
