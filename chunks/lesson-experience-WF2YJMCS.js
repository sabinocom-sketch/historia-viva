import{B as M,D as Q,H as U,a as $,d as C,h as S,k as N,l as n,m as G,n as m,p as r,q as z,r as P,t as H,u as x,v as k,w as I,x as L,y as T,z as A}from"./chunk-JG2SOZLM.js";import{a as V}from"./chunk-UEKKBUIS.js";function _(){let e=n.currentLessonId?L(n.currentLessonId):x(n.currentEra)[0];return e?O(e).mood:""}function Se(){let e=n.currentLessonId?L(n.currentLessonId):x(n.currentEra)[0];if(!e)return"";if(n.currentLessonEntryId!==e.id){n.currentLessonEntryId=e.id,n.currentLessonMode="intro",n.currentLessonStoryBlockIndex=0,n.currentPostStoryStep="summary";let c=re(e.id);n.currentLessonReflectionText=c.reflectionAnswer||"",n.currentLessonDebateChoice=c.debateChoice||"",n.currentLessonQuizAnswers=c.quizAnswers||{},n.currentLessonQuizChoice=null}let a=n.currentLessonId===e.id,t=O(e),o=P($[e.eraKey].curiosities||[],e.index,"Cada vest\xEDgio hist\xF3rico muda quando fazemos uma pergunta melhor."),s=be(e.title,e.detail,e.category,...e.related||[]),i=n.currentLessonMode==="intro";return`
  ...
  `}function J(e,a={}){if(n.currentLessonMode==="story"||n.currentLessonMode==="understand"){let t=g(e.id),o=w(t);return`
    ...
    `}return te(e,a)}function X(e,a,t,o={}){let s=e||{id:"story",visualType:"spark",backgroundMood:"cave-dark",text:"Um pequeno momento come\xE7ou a mudar a experi\xEAncia humana."},i=s.eraKey||o.eraKey||"",c=s.sectionId||o.sectionId||"",l=i==="pre-historia"||["paleolitico","mesolitico","revolucao-neolitica"].includes(c),u=l?Z(s,o):"",p=l?Y(s,o):"",d=s.text,h=ee(d),F=a===0?"eager":"lazy",D=a===0?"high":"auto";return`
    <article class="story-block ${l?"storyblock-prehistory cave-firelight":""}" style="--cave-line-count: ${h}" data-story-block="${r(s.id)}" data-story-index="${a%3}" data-visual="${r(s.visualType)}" data-background="${r(s.backgroundMood)}" data-mood="${r(_())}" data-era="${r(i)}" data-section="${r(c)}" data-prehistory-artifact="${r(u)}">
  ...
  `}function te(e,a={}){let t=se(e,a),o=oe(n.currentLessonMode),s=y.indexOf(o),i=y.length;n.currentPostStoryStep=o;let l={summary:B,reflection:me,debate:ge,quiz:he}[o]||B;return`
  ...
  `}function oe(e){let a=W[e]||e||n.currentPostStoryStep||"summary";return y.includes(a)?a:"summary"}function se(e,a={}){let t=ie(e,a);return{summary:{summary:t.summary,previous:"story"},reflection:{...t.reflection,previous:"summary"},debate:{...t.debate,previous:"reflection"},quiz:{quiz:t.quiz,previous:"debate",nextLessonId:T(e.eraKey,e.id)?.id||""}}}function re(e=""){if(typeof localStorage>"u"||!e)return{};try{return JSON.parse(localStorage.getItem("historiaVivaProgress"))?.lessonProgress?.[e]||{}}catch{return{}}}function ie(e,a={}){if(e.postLesson)return e.postLesson;let t=b(e.title),o=k(e.id),s=I(e),i=ne(e,o,s);return{summary:i,reflection:{question:s.reflection||a.insight||`Que mudanca historica te parece mais importante em ${t}?`},debate:ce(e,t,s),quiz:le(e,i)}}function ne(e,a=[],t={}){let s=[t.keyTakeaway,...t.evidence||[],...t.archaeology||[],...t.consequences||[],...a.map(i=>i.text),e.detail].filter(Boolean).map(i=>v(i,15)).filter(Boolean).filter((i,c,l)=>l.findIndex(u=>m(u)===m(i))===c).slice(0,5);for(;s.length<3;)s.push(v(`${b(e.title)} ajuda a perceber causas, escolhas humanas e consequencias.`,14));return s.slice(0,5)}function ce(e,a,t={}){let o=v(t.keyTakeaway||e.detail||a,18);return{question:`Como devemos interpretar ${a}: sobretudo como progresso ou como mudanca complexa?`,options:[{id:"optionA",label:"Foi sobretudo progresso",feedback:`Esta posicao pode indicar que ${o.toLowerCase()} abriu possibilidades importantes. Ainda assim, muitos historiadores defendem que ganhos materiais tambem trouxeram novos custos.`},{id:"optionB",label:"Foi uma mudanca complexa",feedback:`Esta posicao sugere que ${o.toLowerCase()} deve ser lido com cautela. Ha debate sobre o peso dos beneficios e dos limites em diferentes grupos sociais.`}]}}function le(e,a=[]){let t=M(e.eraKey),o=Q(e.eraKey,e.id),i=(t.length?Array.from({length:Math.min(3,t.length)},(c,l)=>t[(o+l)%t.length]):[]).map(c=>ue(c));for(;i.length<3;){let c=a[i.length%Math.max(1,a.length)]||e.detail||e.title;i.push({question:"Que ideia resume melhor esta licao?",options:[v(c,12),"Aconteceu sem causas historicas.","Teve apenas uma consequencia simples.","Nao pode ser estudada por vestigios."],correctIndex:0,explanation:"A melhor resposta liga a licao a causas, evidencias e consequencias."})}return i.slice(0,5)}function ue(e={}){return{question:e.question||"Que ideia fica desta licao?",options:de(e.options||[]),correctIndex:Number.isInteger(e.correctIndex)?e.correctIndex:Math.min(Math.max(e.answer||0,0),3),explanation:e.explanation||"Esta resposta enquadra melhor o contexto historico da licao."}}function de(e=[]){let a=["Foi um processo sem contexto.","Aconteceu da mesma forma em todo o lado.","Teve apenas uma causa.","Deve ser lido com fontes e consequencias."],t=e.slice(0,4);return a.forEach(o=>{t.length<4&&!t.includes(o)&&t.push(o)}),t.slice(0,4)}function j(e=""){let a=String(e||"").replace(/\s+/g," ").trim();return a?a.match(/[^.!?…]+(?:[.!?…]+["”’»]?|$)/g)?.map(t=>t.trim()).filter(Boolean)||[a]:[]}function R(e=""){return String(e||"").trim().split(/\s+/).filter(Boolean).length}function v(e="",a=24){let t=j(e);if(!t.length)return"";let o=[],s=0;for(let i of t){let c=R(i);if(o.length&&s+c>a||(o.push(i),s+=c,s>=a))break}return o.join(" ")}function pe(e,a){let t=j(e?.text);if(!t.length)return[e];let o=[],s=[],i=0;return t.forEach(c=>{let l=R(c);s.length&&i+l>a&&(o.push(s.join(" ")),s=[],i=0),s.push(c),i+=l}),s.length&&o.push(s.join(" ")),o.map((c,l)=>({...e,id:`${e.id||"story"}-${l+1}`,text:c}))}function g(e){return k(e).flatMap(t=>pe(t,55))}function E(e,a,t=""){let o=g(e.id).length,s=Math.max(1,o+y.length),i=Math.min(Math.max(a,0),s-1),c=s<=1?100:Math.round(i/(s-1)*100);return`
  ...
  `}function f(e,a,t){let o=g(t.id).length;return E(t,o+e,"post-story-progress")}function q(e,a="Continuar",t=!1){return`
  ...
  `}function B(e,a,t,o){let s=(e.summary||[]).slice(0,5);return`
  ...
  `}function ge(e,a,t,o){let s=(e.options||[]).find(i=>i.id===n.currentLessonDebateChoice);return`
  ...
  `}function he(e,a,t,o){let s=e.quiz||[],i=n.currentLessonQuizAnswers||{},c=s.reduce((p,d,h)=>p+(i[h]===d.correctIndex?1:0),0),l=s.filter((p,d)=>i[d]!==void 0).length,u=s.length>0&&l===s.length;return`
    <article class="post-story-screen challenge-screen quiz-screen" data-era="${r(a.eraKey||"")}" data-section="${r(a.sectionId||"")}" data-theme="${r(a.category)}">
      ...
      <div class="story-block-actions lesson-screen-actions post-story-actions">
        ...
        ${e.nextLessonId?`<button type="button" data-lesson-action="next-lesson" data-next-lesson="${r(e.nextLessonId)}" ${u?"":"disabled"}>Avancar para a proxima licao</button>`:`<button type="button" data-lesson-action="complete" ${u?"":"disabled"}>Concluir licao</button>`}
      </div>
    </article>
