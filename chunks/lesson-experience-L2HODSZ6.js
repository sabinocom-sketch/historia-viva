import{B as T,D as A,H as V,a as h,d as C,h as I,k as E,l as n,m as Q,n as u,p as t,q as f,r as q,t as O,u as b,v as y,w as P,x as $,y as S,z}from"./chunk-VNVLKROU.js";import{a as R}from"./chunk-UEKKBUIS.js";function N(){let e=n.currentLessonId?$(n.currentLessonId):b(n.currentEra)[0];return e?L(e).mood:""}function xe(){let e=n.currentLessonId?$(n.currentLessonId):b(n.currentEra)[0];if(!e)return"";n.currentLessonEntryId!==e.id&&(n.currentLessonEntryId=e.id,n.currentLessonMode="intro",n.currentLessonStoryBlockIndex=0,n.currentPostStoryStep="reflection",n.currentLessonQuizChoice=null);let a=n.currentLessonId===e.id,o=L(e),r=q(h[e.eraKey].curiosities||[],e.index,"Cada vest\xEDgio hist\xF3rico muda quando fazemos uma pergunta melhor."),s=me(e.title,e.detail,e.category,...e.related||[]),i=n.currentLessonMode==="intro";return`
    <section class="active-lesson-panel lesson-view ${a?"is-active":""}" data-active-lesson="${t(e.id)}" data-era="${t(e.eraKey)}" data-section="${t(e.sectionId||"")}" data-mood="${t(o.mood)}" data-mode="${t(n.currentLessonMode)}" data-theme="${t(e.category)}">
      ...
      </div>`:D(e,{insight:s,curiosity:r})}
    </section>
  `}function D(e,a={}){if(n.currentLessonMode==="story"||n.currentLessonMode==="understand"){let o=y(e.id),r=x(o);return`
    ...
    <article class="story-block ${l?"storyblock-prehistory cave-firelight":""}" style="--cave-line-count: ${w}" data-story-block="${t(s.id)}" data-story-index="${a%3}" data-visual="${t(s.visualType)}" data-background="${t(s.backgroundMood)}" data-mood="${t(N())}" data-era="${t(i)}" data-section="${t(c)}" data-prehistory-artifact="${t(d)}">
      ...
      <div class="story-block-progress prehistory-progress" aria-label="Progresso dos story blocks">
        ${Array.from({length:o},(ye,j)=>`<span class="${j===a?"active":""}" aria-hidden="true"></span>`).join("")}
      </div>
  ...
  `}function G(e={},a={}){let o=u(`${a.title||""} ${e.text||""} ${e.visualType||""}`);return[{keywords:["fogo","chama","fogueira","aquecer","cozinhar"],title:"A Descoberta do Fogo"},{keywords:["pedra","ferramenta","silex","raspador","cortar"],title:"A Pedra que se Tornou Ferramenta"},{keywords:["caca","pesca","animal","rasto","lanca"],title:"A Ca\xE7a Pintada na Rocha"},{keywords:["abrigo","gruta","caverna"],title:"O Primeiro Abrigo"},{keywords:["arte","rupestre","pintura","pigmento","mao"],title:"A Mem\xF3ria na Parede"},{keywords:["aldeia","agricultura","sementes","cultivo"],title:"Quando a Terra Virou Casa"}].find(s=>s.keywords.some(i=>o.includes(i)))?.title||m(a.title||"Marca na Pedra")}function U(e={},a={}){let o=u(`${a.title||""} ${e.text||""} ${e.visualType||""}`);return["fogo","chama","fogueira","aquecer","cozinhar","spark","embers"].some(r=>o.includes(r))?"fire":["caca","pesca","animal","rasto","lanca","spear","tracks"].some(r=>o.includes(r))?"hunt":["arte","rupestre","pintura","pigmento","mao","hand"].some(r=>o.includes(r))?"painting":["abrigo","gruta","caverna"].some(r=>o.includes(r))?"shelter":"flint"}function _(e=""){let a=String(e||"").trim();if(!a)return[""];let o=a.split(/(?<=[.!?])\s+|;\s+/).map(r=>r.trim()).filter(Boolean);return o.length?o:[a]}function B(e=""){let a=String(e||"").trim().split(/\n{2,}/).map(s=>s.trim()).filter(Boolean),o=a.length?a:[String(e||"").trim()],r=[];return o.forEach(s=>{let i=_(s);if(i.length<=2){r.push(i);return}for(let c=0;c<i.length;c+=2)r.push(i.slice(c,c+2))}),r.length?r:[[""]]}function J(e=""){return B(e).reduce((a,o)=>a+o.length,0)}function X(e=""){let a=0;return B(e).map((o,r)=>`
      <span class="cave-reveal-paragraph" style="--cave-paragraph-index: ${r}" aria-hidden="true">
        ${o.map(s=>{let i=a;return a+=1,`
            <span class="cave-reveal-line" style="--cave-line-index: ${i}" aria-hidden="true">
              ${t(s)}
            </span>
          `}).join("")}
      </span>
    `).join("")}function W(e,a={}){let o=Z(e,a),r=Y(n.currentLessonMode),s=k.indexOf(r),i=k.length;n.currentPostStoryStep=r;let l={reflection:M,assimilation:ne,reality:ie,critical:ce,challenge:le,reward:de,nextTeaser:ue}[r]||M;return`
  ...
  `}function Y(e){let a=F[e]||e||n.currentPostStoryStep||"reflection";return k.includes(a)?a:"reflection"}function Z(e,a={}){let o=L(e),r=m(e.title),s=y(e.id),i=P(e),c=S(e.eraKey,e.id);return{reflection:ee(e,a.insight,i),assimilation:ae(e,s,r,i),reality:te(e,r,i),critical:oe(e,r,i),challenge:re(e),reward:{kicker:"Artefacto desbloqueado",title:"Mem\xF3ria guardada",text:i.keyTakeaway||`Guardaste uma chave de leitura sobre ${r}.`,artifact:o.preview?.[0]||"Artefacto narrativo desbloqueado",previous:"challenge"},nextTeaser:{kicker:"Pr\xF3xima porta temporal",title:c?m(c.title):"Rever a jornada",text:c?se(r,m(c.title)):"A viagem continua quando voltares ao mapa da era.",nextLessonId:c?.id||"",previous:"reward"}}}function ee(e,a="",o={}){let r=m(e.title),s=u(r);return{kicker:"Pausa de assimila\xE7\xE3o",title:"O que isto mudou?",text:o.reflection||(s.includes("fogo")?"O fogo nao mudou apenas a sobrevivencia. Mudou a forma como os humanos viviam juntos.":f(a||e.detail||K(e),150)),previous:"story"}}function ae(e,a,o,r={}){let s=[...r.evidence||[],...r.archaeology||[]],i=s.length?s.slice(0,3):a.slice(0,3).map(c=>c.text);return{kicker:"Guia hist\xF3rico",title:"O que acabaste de descobrir?",text:r.keyTakeaway||`${o} fica mais claro quando separas descoberta, escolha e consequencia.`,prompts:i.length?i:["Que problema humano aparece aqui?","Que decisao mudou a vida diaria?","Que consequencia veio depois?"],previous:"reflection"}}function te(e,a,o={}){let s=u(a).includes("fogo")?[["Energia","Hoje continuamos dependentes de energia para viver, trabalhar e comunicar."],["Tecnologia","Uma ferramenta simples pode reorganizar uma comunidade inteira."],["Seguranca","Proteger recursos importantes continua a ser uma decisao coletiva."]]:[["Tecnologia","Mudancas tecnicas alteram rotinas, trabalho e relacoes sociais."],["Comunidade","O que uma sociedade aprende muda a forma como coopera."],["Escolhas","Cada avancar tambem cria dependencias e novos custos."]];return{kicker:"Ponte ao presente",title:"Do passado para hoje",text:o.presentConnection||`${a} nao ficou preso ao passado. A mesma logica ainda aparece no presente.`,cards:s,previous:"assimilation"}}function oe(e,a,o={}){let s=u(a).includes("fogo")?"O dominio do fogo foi apenas progresso?":"Esta mudanca trouxe so beneficios?";return{kicker:"Lente cr\xEDtica",title:o.reflection||s,text:o.curiosity||"Olha para o mesmo momento por lentes diferentes.",perspectives:[["Sobrevivencia","Ajudou pessoas a resistir melhor ao mundo."],["Vida social","Mudou a forma como grupos se organizavam."],["Custo","Tambem criou novas pressoes sobre recursos e ambiente."]],previous:"reality"}}function re(e){let a=T(e.eraKey),o=a[A(e.eraKey,e.id)]||a[0];return{kicker:"Desafio",title:o?.question||"Que escolha ajudaria melhor o grupo?",quiz:o,previous:"critical"}}function se(e,a){return`${e} abriu uma porta. A pr\xF3xima li\xE7\xE3o mostra o que mudou a seguir: ${a}.`}function p(e,a){return`
  ...
  `}function le(e,a,o,r){let s=e.quiz||{question:e.title,options:[],answer:0,explanation:""},i=n.currentLessonQuizChoice!==null,c=i&&n.currentLessonQuizChoice===s.answer;return`
    <article class="post-story-screen challenge-screen" data-era="${t(a.eraKey||"")}" data-section="${t(a.sectionId||"")}" data-theme="${t(a.category)}">
      <span class="post-story-background" aria-hidden="true"></span>
      <div class="post-story-copy">
        <span class="post-story-kicker">${t(e.kicker)}</span>
        <h3>${t(e.title)}</h3>
        <p>${t(i?`${c?"Boa leitura.":"Quase."} ${s.explanation}`:"Escolhe pelo contexto, n\xE3o pela mem\xF3ria.")}</p>
      </div>
      <div class="lesson-quiz-options challenge-options">
        ${s.options.map((l,d)=>`
          <button class="${i&&d===s.answer?"correct":""} ${i&&d===n.currentLessonQuizChoice&&d!==s.answer?"wrong":""}" type="button" data-lesson-quiz-option="${d}">
            ${t(l)}
          </button>
        `).join("")}
      </div>
