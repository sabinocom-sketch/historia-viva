import{A as S,C as z,G as E,a as b,d as L,h as w,k as T,l as s,m as A,n as u,p as t,q as g,r as I,t as j,u as h,v,w as $,x as q,y as C}from"./chunk-74ES6JHX.js";import{a as B}from"./chunk-UEKKBUIS.js";function O(){let e=s.currentLessonId?$(s.currentLessonId):h(s.currentEra)[0];return e?x(e).mood:""}function pe(){let e=s.currentLessonId?$(s.currentLessonId):h(s.currentEra)[0];if(!e)return"";s.currentLessonEntryId!==e.id&&(s.currentLessonEntryId=e.id,s.currentLessonMode="intro",s.currentLessonStoryBlockIndex=0,s.currentPostStoryStep="reflection",s.currentLessonQuizChoice=null);let a=s.currentLessonId===e.id,o=x(e),r=I(b[e.eraKey].curiosities||[],e.index,"Cada vest\xEDgio hist\xF3rico muda quando fazemos uma pergunta melhor."),n=re(e.title,e.detail,e.category,...e.related||[]),i=s.currentLessonMode==="intro";return`
      ...
      ${i?`<div class="lesson-hero">
        ...
        <div class="lesson-hero-copy">
          ...
          <div class="lesson-preview-list" aria-label="Preview da li\xE7\xE3o">
            ${o.preview.map(l=>`<span>${t(l)}</span>`).join("")}
          </div>
          <button type="button" class="lesson-start-button" data-lesson-action="story">Iniciar Jornada</button>
        </div>
      </div>`:R(e,{insight:n,curiosity:r})}
    </section>
  `}function R(e,a={}){if(s.currentLessonMode==="story"||s.currentLessonMode==="understand"){let o=v(e.id),r=k(o);return`
    ...
    `}return F(e,a)}function V(e,a,o,r={}){let n=e||{id:"story",visualType:"spark",backgroundMood:"cave-dark",text:"Um pequeno momento come\xE7ou a mudar a experi\xEAncia humana."},i=n.eraKey||r.eraKey||"",l=n.sectionId||r.sectionId||"";return`
    <article class="story-block" data-story-block="${t(n.id)}" data-visual="${t(n.visualType)}" data-background="${t(n.backgroundMood)}" data-mood="${t(O())}" data-era="${t(i)}" data-section="${t(l)}">
  ...
  `}function F(e,a={}){let o=D(e,a),r=N(s.currentLessonMode),n=f.indexOf(r),i=f.length;s.currentPostStoryStep=r;let d={reflection:P,assimilation:W,reality:Y,critical:Z,challenge:ee,reward:ae,nextTeaser:te}[r]||P;return`
  ...
  `}function N(e){let a=Q[e]||e||s.currentPostStoryStep||"reflection";return f.includes(a)?a:"reflection"}function D(e,a={}){let o=x(e),r=y(e.title),n=v(e.id),i=q(e.eraKey,e.id);return{reflection:H(e,a.insight),assimilation:G(e,n,r),reality:U(e,r),critical:_(e,r),challenge:J(e),reward:{kicker:"Artefacto desbloqueado",title:"Mem\xF3ria guardada",text:`Guardaste uma chave de leitura sobre ${r}.`,artifact:o.preview?.[0]||"Artefacto narrativo desbloqueado",previous:"challenge"},nextTeaser:{kicker:"Pr\xF3xima porta temporal",title:i?y(i.title):"Rever a jornada",text:i?X(r,y(i.title)):"A viagem continua quando voltares ao mapa da era.",nextLessonId:i?.id||"",previous:"reward"}}}function H(e,a=""){let o=y(e.title);return{kicker:"Pausa de assimila\xE7\xE3o",title:"O que isto mudou?",text:u(o).includes("fogo")?"O fogo nao mudou apenas a sobrevivencia. Mudou a forma como os humanos viviam juntos.":g(a||e.detail||K(e),150),previous:"story"}}function G(e,a,o){let r=a.slice(0,3).map(n=>n.text);return{kicker:"Guia hist\xF3rico",title:"O que acabaste de descobrir?",text:`${o} fica mais claro quando separas descoberta, escolha e consequencia.`,prompts:r.length?r:["Que problema humano aparece aqui?","Que decisao mudou a vida diaria?","Que consequencia veio depois?"],previous:"reflection"}}function U(e,a){let r=u(a).includes("fogo")?[["Energia","Hoje continuamos dependentes de energia para viver, trabalhar e comunicar."],["Tecnologia","Uma ferramenta simples pode reorganizar uma comunidade inteira."],["Seguranca","Proteger recursos importantes continua a ser uma decisao coletiva."]]:[["Tecnologia","Mudancas tecnicas alteram rotinas, trabalho e relacoes sociais."],["Comunidade","O que uma sociedade aprende muda a forma como coopera."],["Escolhas","Cada avancar tambem cria dependencias e novos custos."]];return{kicker:"Ponte ao presente",title:"Do passado para hoje",text:`${a} nao ficou preso ao passado. A mesma logica ainda aparece no presente.`,cards:r,previous:"assimilation"}}function _(e,a){return{kicker:"Lente cr\xEDtica",title:u(a).includes("fogo")?"O dominio do fogo foi apenas progresso?":"Esta mudanca trouxe so beneficios?",text:"Olha para o mesmo momento por lentes diferentes.",perspectives:[["Sobrevivencia","Ajudou pessoas a resistir melhor ao mundo."],["Vida social","Mudou a forma como grupos se organizavam."],["Custo","Tambem criou novas pressoes sobre recursos e ambiente."]],previous:"reality"}}function J(e){let a=S(e.eraKey),o=a[z(e.eraKey,e.id)]||a[0];return{kicker:"Desafio",title:o?.question||"Que escolha ajudaria melhor o grupo?",quiz:o,previous:"critical"}}function X(e,a){return`${e} abriu uma porta. A pr\xF3xima li\xE7\xE3o mostra o que mudou a seguir: ${a}.`}function p(e,a){return`
  ...
  `}function ee(e,a,o,r){let n=e.quiz||{question:e.title,options:[],answer:0,explanation:""},i=s.currentLessonQuizChoice!==null,l=i&&s.currentLessonQuizChoice===n.answer;return`
    <article class="post-story-screen challenge-screen" data-era="${t(a.eraKey||"")}" data-section="${t(a.sectionId||"")}" data-theme="${t(a.category)}">
      <span class="post-story-background" aria-hidden="true"></span>
      <div class="post-story-copy">
        <span class="post-story-kicker">${t(e.kicker)}</span>
        <h3>${t(e.title)}</h3>
        <p>${t(i?`${l?"Boa leitura.":"Quase."} ${n.explanation}`:"Escolhe pelo contexto, n\xE3o pela mem\xF3ria.")}</p>
      </div>
