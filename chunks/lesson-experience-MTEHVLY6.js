import{B as M,D as A,H as D,a as h,d as I,h as P,k as V,l as i,m as F,n as u,p as t,q as f,r as S,t as N,u as b,v,w as q,x as $,y as z,z as T}from"./chunk-VNVLKROU.js";import{a as O}from"./chunk-UEKKBUIS.js";function G(){let e=i.currentLessonId?$(i.currentLessonId):b(i.currentEra)[0];return e?w(e).mood:""}function Ce(){let e=i.currentLessonId?$(i.currentLessonId):b(i.currentEra)[0];if(!e)return"";i.currentLessonEntryId!==e.id&&(i.currentLessonEntryId=e.id,i.currentLessonMode="intro",i.currentLessonStoryBlockIndex=0,i.currentPostStoryStep="reflection",i.currentLessonQuizChoice=null);let a=i.currentLessonId===e.id,o=w(e),r=S(h[e.eraKey].curiosities||[],e.index,"Cada vest\xEDgio hist\xF3rico muda quando fazemos uma pergunta melhor."),s=ge(e.title,e.detail,e.category,...e.related||[]),n=i.currentLessonMode==="intro";return`
    <section class="active-lesson-panel lesson-view ${a?"is-active":""}" data-active-lesson="${t(e.id)}" data-era="${t(e.eraKey)}" data-section="${t(e.sectionId||"")}" data-mood="${t(o.mood)}" data-mode="${t(i.currentLessonMode)}" data-theme="${t(e.category)}">
      ...
      </div>`:U(e,{insight:s,curiosity:r})}
    </section>
  `}function U(e,a={}){if(i.currentLessonMode==="story"||i.currentLessonMode==="understand"){let o=v(e.id),r=x(o);return`
    ...
    `}return ee(e,a)}function _(e,a,o,r={}){let s=e||{id:"story",visualType:"spark",backgroundMood:"cave-dark",text:"Um pequeno momento come\xE7ou a mudar a experi\xEAncia humana."},n=s.eraKey||r.eraKey||"",c=s.sectionId||r.sectionId||"",l=n==="pre-historia"||["paleolitico","mesolitico","revolucao-neolitica"].includes(c),d=l?X(s,r):"",g=l?J(s,r):"",L=Y(s.text),R=a===0?"eager":"lazy",E=a===0?"high":"auto",C=o>1?Math.round(a/(o-1)*100):100;return`
    <article class="story-block ${l?"storyblock-prehistory cave-firelight":""}" style="--cave-line-count: ${L}; --story-progress-pct: ${C}%" data-story-block="${t(s.id)}" data-story-index="${a%3}" data-visual="${t(s.visualType)}" data-background="${t(s.backgroundMood)}" data-mood="${t(G())}" data-era="${t(n)}" data-section="${t(c)}" data-prehistory-artifact="${t(d)}">
      ...
      <span class="story-block-visual cave-visual-artifact prehistory-artifact" aria-hidden="true">
        ${d==="flint"?`<picture class="prehistory-flint-picture">
          ...
          <img class="prehistory-flint-sprite" src="assets/silex-lascado-sprite-desktop.webp?v=20260526" alt="" width="1024" height="525" loading="${R}" decoding="async" fetchpriority="${E}" />
        </picture>`:""}
      </span>
      ...
      <div class="story-block-progress prehistory-progress" aria-label="Story block ${a+1} de ${o}">
        ...
        <span class="story-progress-percent">${C}%</span>
        <span class="story-progress-segments" aria-hidden="true">
          ${Array.from({length:o},(fe,Q)=>`<span class="story-progress-segment ${Q===a?"active":""}"></span>`).join("")}
        </span>
      </div>
  ...
  `}function J(e={},a={}){let o=u(`${a.title||""} ${e.text||""} ${e.visualType||""}`);return[{keywords:["fogo","chama","fogueira","aquecer","cozinhar"],title:"A Descoberta do Fogo"},{keywords:["pedra","ferramenta","silex","raspador","cortar"],title:"A Pedra que se Tornou Ferramenta"},{keywords:["caca","pesca","animal","rasto","lanca"],title:"A Ca\xE7a Pintada na Rocha"},{keywords:["abrigo","gruta","caverna"],title:"O Primeiro Abrigo"},{keywords:["arte","rupestre","pintura","pigmento","mao"],title:"A Mem\xF3ria na Parede"},{keywords:["aldeia","agricultura","sementes","cultivo"],title:"Quando a Terra Virou Casa"}].find(s=>s.keywords.some(n=>o.includes(n)))?.title||m(a.title||"Marca na Pedra")}function X(e={},a={}){let o=u(`${a.title||""} ${e.text||""} ${e.visualType||""}`);return["fogo","chama","fogueira","aquecer","cozinhar","spark","embers"].some(r=>o.includes(r))?"fire":["caca","pesca","animal","rasto","lanca","spear","tracks"].some(r=>o.includes(r))?"hunt":["arte","rupestre","pintura","pigmento","mao","hand"].some(r=>o.includes(r))?"painting":["abrigo","gruta","caverna"].some(r=>o.includes(r))?"shelter":"flint"}function W(e=""){let a=String(e||"").trim();if(!a)return[""];let o=a.split(/(?<=[.!?])\s+|;\s+/).map(r=>r.trim()).filter(Boolean);return o.length?o:[a]}function K(e=""){let a=String(e||"").trim().split(/\n{2,}/).map(s=>s.trim()).filter(Boolean),o=a.length?a:[String(e||"").trim()],r=[];return o.forEach(s=>{let n=W(s);if(n.length<=2){r.push(n);return}for(let c=0;c<n.length;c+=2)r.push(n.slice(c,c+2))}),r.length?r:[[""]]}function Y(e=""){return K(e).reduce((a,o)=>a+o.length,0)}function Z(e=""){let a=0;return K(e).map((o,r)=>`
    ...
    `).join("")}function ee(e,a={}){let o=te(e,a),r=ae(i.currentLessonMode),s=k.indexOf(r),n=k.length;i.currentPostStoryStep=r;let l={reflection:B,assimilation:le,reality:de,critical:ue,challenge:pe,reward:me,nextTeaser:ye}[r]||B;return`
  ...
  `}function y(e,a="Continuar",o=!1){return`
  ...
  `}function pe(e,a,o,r){let s=e.quiz||{question:e.title,options:[],answer:0,explanation:""},n=i.currentLessonQuizChoice!==null,c=n&&i.currentLessonQuizChoice===s.answer;return`
