import{B as j,D as R,H as _,a as L,d as I,h as T,k as H,l as i,m as U,n as p,p as r,q as A,r as M,t as W,u as q,v as w,w as Q,x as S,y as B,z as K}from"./chunk-FA56LTI6.js";import{a as G}from"./chunk-UEKKBUIS.js";function X(){let e=i.currentLessonId?S(i.currentLessonId):q(i.currentEra)[0];return e?N(e).mood:""}function Ie(){let e=i.currentLessonId?S(i.currentLessonId):q(i.currentEra)[0];if(!e)return"";if(i.currentLessonEntryId!==e.id){i.currentLessonEntryId=e.id,i.currentLessonMode="intro",i.currentLessonStoryBlockIndex=0,i.currentPostStoryStep="summary";let c=ie(e.id);i.currentLessonReflectionText=c.reflectionAnswer||"",i.currentLessonDebateChoice=c.debateChoice||"",i.currentLessonQuizAnswers=c.quizAnswers||{},i.currentPostLessonQuizIndex=0,i.currentLessonQuizChoice=null}let a=i.currentLessonId===e.id,t=N(e),o=M(L[e.eraKey].curiosities||[],e.index,"Cada vest\xEDgio hist\xF3rico muda quando fazemos uma pergunta melhor."),s=ke(e.title,e.detail,e.category,...e.related||[]),n=i.currentLessonMode==="intro";return`
    <section class="active-lesson-panel lesson-view ${a?"is-active":""}" data-active-lesson="${r(e.id)}" data-era="${r(e.eraKey)}" data-section="${r(e.sectionId||"")}" data-mood="${r(t.mood)}" data-mode="${r(i.currentLessonMode)}" data-theme="${r(e.category)}">
      ...
      </div>`:Y(e,{insight:s,curiosity:o})}
    </section>
  `}function Y(e,a={}){if(i.currentLessonMode==="story"||i.currentLessonMode==="understand"){let t=g(e.id),o=P(t);return`
    ...
    `}return se(e,a)}function Z(e,a,t,o={}){let s=e||{id:"story",visualType:"spark",backgroundMood:"cave-dark",text:"Um pequeno momento come\xE7ou a mudar a experi\xEAncia humana."},n=s.eraKey||o.eraKey||"",c=s.sectionId||o.sectionId||"",u=n==="pre-historia"||["paleolitico","mesolitico","revolucao-neolitica"].includes(c),l=u?ae(s,o):"",d=u?ee(s,o):"",m=s.text,h=te(m),x=a===0?"eager":"lazy",v=a===0?"high":"auto";return`
    <article class="story-block ${u?"storyblock-prehistory cave-firelight":""}" style="--cave-line-count: ${h}" data-story-block="${r(s.id)}" data-story-index="${a%3}" data-visual="${r(s.visualType)}" data-background="${r(s.backgroundMood)}" data-mood="${r(X())}" data-era="${r(n)}" data-section="${r(c)}" data-prehistory-artifact="${r(l)}">
      ...
      <span class="story-block-visual cave-visual-artifact prehistory-artifact" aria-hidden="true">
        ${l==="flint"?`<picture class="prehistory-flint-picture">
          ...
          <img class="prehistory-flint-sprite" src="assets/silex-desktop.png?v=20260528" alt="" width="1024" height="1024" loading="${x}" decoding="async" fetchpriority="${v}" />
        </picture>`:""}
      </span>
  ...
  `}function ee(e={},a={}){return $(a.title||e.title||"Marca na Pedra")}function ae(e={},a={}){let t=p(`${a.title||""} ${e.text||""} ${e.visualType||""}`);return["fogo","chama","fogueira","aquecer","cozinhar","spark","embers"].some(o=>t.includes(o))?"fire":["caca","pesca","animal","rasto","lanca","spear","tracks"].some(o=>t.includes(o))?"hunt":["arte","rupestre","pintura","pigmento","mao","hand"].some(o=>t.includes(o))?"painting":["abrigo","gruta","caverna"].some(o=>t.includes(o))?"shelter":"flint"}function E(e=""){let a=String(e||"").trim();if(!a)return[""];let t=a.split(/(?<=[.!?])\s+|;\s+/).map(o=>o.trim()).filter(Boolean);return t.length?t:[a]}function te(e=""){return E(e).length}function oe(e=""){return`
  ...
  `}function se(e,a={}){let t=ne(e,a),o=re(i.currentLessonMode),s=y.indexOf(o),n=y.length;i.currentPostStoryStep=o;let u={summary:O,reflection:ve,debate:ye,quiz:fe}[o]||O;return`
  ...
  `}function re(e){let a=J[e]||e||i.currentPostStoryStep||"summary";return y.includes(a)?a:"summary"}function ne(e,a={}){let t=ce(e,a);return{summary:{summary:t.summary,previous:"story"},reflection:{...t.reflection,previous:"summary"},debate:{...t.debate,previous:"reflection"},quiz:{quiz:t.quiz,previous:"debate",nextLessonId:B(e.eraKey,e.id)?.id||""}}}function ie(e=""){if(typeof localStorage>"u"||!e)return{};try{return JSON.parse(localStorage.getItem("historiaVivaProgress"))?.lessonProgress?.[e]||{}}catch{return{}}}function ce(e,a={}){if(e.postLesson)return e.postLesson;let t=$(e.title),o=w(e.id),s=Q(e),n=ue(e,o,s);return{summary:n,reflection:{question:s.reflection||a.insight||`Que mudanca historica te parece mais importante em ${t}?`},debate:de(e,t,s),quiz:pe(e,n)}}function ue(e,a=[],t={}){let s=[t.keyTakeaway,...t.evidence||[],...t.archaeology||[],...t.consequences||[],...a.map(n=>n.text),e.detail].filter(Boolean).map(n=>le(f(n,15),e)).filter(Boolean).filter((n,c,u)=>u.findIndex(l=>p(l)===p(n))===c).slice(0,4);for(;s.length<3;)s.push(f(`${$(e.title)} ajuda a perceber causas, escolhas humanas e consequencias.`,14));return s.slice(0,4)}function le(e="",a={}){if(!e||C(e)>=9)return e;let t=p(`${a.title||""} ${a.detail||""}`);return t.includes("pedra")||t.includes("ferramenta")?`${e} O gesto humano dava-lhe funcao e sentido.`:t.includes("fogo")?`${e} O uso repetido transformava seguranca, alimento e convivio.`:`${e} O contexto historico ajuda a perceber a mudanca.`}function de(e,a,t={}){let o=f(t.keyTakeaway||e.detail||a,18);return{question:`Como devemos interpretar ${a}: sobretudo como progresso ou como mudanca complexa?`,options:[{id:"optionA",label:"Foi sobretudo progresso",feedback:`Esta posicao pode indicar que ${o.toLowerCase()} abriu possibilidades importantes. Ainda assim, muitos historiadores defendem que ganhos materiais tambem trouxeram novos custos.`},{id:"optionB",label:"Foi uma mudanca complexa",feedback:`Esta posicao sugere que ${o.toLowerCase()} deve ser lido com cautela. Ha debate sobre o peso dos beneficios e dos limites em diferentes grupos sociais.`}]}}function pe(e,a=[]){let t=j(e.eraKey),o=R(e.eraKey,e.id),n=(t.length?Array.from({length:Math.min(3,t.length)},(c,u)=>t[(o+u)%t.length]):[]).map(c=>me(c));for(;n.length<3;){let c=a[n.length%Math.max(1,a.length)]||e.detail||e.title;n.push({question:"Que ideia resume melhor esta licao?",options:[f(c,12),"Aconteceu sem causas historicas.","Teve apenas uma consequencia simples.","Nao pode ser estudada por vestigios."],correctIndex:0,explanation:"A melhor resposta liga a licao a causas, evidencias e consequencias."})}return n.slice(0,5)}function me(e={}){return{question:e.question||"Que ideia fica desta licao?",options:ge(e.options||[]),correctIndex:Number.isInteger(e.correctIndex)?e.correctIndex:Math.min(Math.max(e.answer||0,0),3),explanation:e.explanation||"Esta resposta enquadra melhor o contexto historico da licao."}}function ge(e=[]){let a=["Foi um processo sem contexto.","Aconteceu da mesma forma em todo o lado.","Teve apenas uma causa.","Deve ser lido com fontes e consequencias."],t=e.slice(0,4);return a.forEach(o=>{t.length<4&&!t.includes(o)&&t.push(o)}),t.slice(0,4)}function F(e=""){let a=String(e||"").replace(/\s+/g," ").trim();return a?a.match(/[^.!?…]+(?:[.!?…]+["”’»]?|$)/g)?.map(t=>t.trim()).filter(Boolean)||[a]:[]}function C(e=""){return String(e||"").trim().split(/\s+/).filter(Boolean).length}function f(e="",a=24){let t=F(e);if(!t.length)return"";let o=[],s=0;for(let n of t){let c=C(n);if(o.length&&s+c>a||(o.push(n),s+=c,s>=a))break}return o.join(" ")}function he(e,a){let t=F(e?.text);if(!t.length)return[e];let o=[],s=[],n=0;return t.forEach(c=>{let u=C(c);s.length&&n+u>a&&(o.push(s.join(" ")),s=[],n=0),s.push(c),n+=u}),s.length&&o.push(s.join(" ")),o.map((c,u)=>({...e,id:`${e.id||"story"}-${u+1}`,text:c}))}function g(e){return w(e).flatMap(t=>he(t,55))}function D(e,a,t=""){let o=g(e.id).length,s=Math.max(1,o+y.length),n=Math.min(Math.max(a,0),s-1),c=s<=1?100:Math.round(n/(s-1)*100);return`
  ...
  `}function b(e,a,t){let o=g(t.id).length;return D(t,o+e,"post-story-progress")}function z(e,a="Continuar",t=!1){return`
  ...
  `}function ye(e,a,t,o){let s=(e.options||[]).find(n=>n.id===i.currentLessonDebateChoice);return`
  ...
  `}function fe(e,a,t,o){let s=e.quiz||[],n=i.currentLessonQuizAnswers||{},c=s.reduce((v,k,V)=>v+(n[V]===k.correctIndex?1:0),0),u=s.filter((v,k)=>n[k]!==void 0).length,l=s.length>0&&u===s.length,d=l?Math.max(0,s.length-1):Math.min(Math.max(Number(i.currentPostLessonQuizIndex)||0,0),Math.max(0,s.length-1)),m=s[d],h=m&&n[d]!==void 0,x=d<s.length-1;return`
    <article class="post-story-screen challenge-screen quiz-screen" data-era="${r(a.eraKey||"")}" data-section="${r(a.sectionId||"")}" data-theme="${r(a.category)}" data-quiz-total="${s.length}">
      ...
      <div class="story-block-actions lesson-screen-actions post-story-actions">
        ...
        ${h&&!l&&x?'<button type="button" data-lesson-action="quiz-next">Proxima pergunta</button>':e.nextLessonId?`<button type="button" data-lesson-action="next-lesson" data-next-lesson="${r(e.nextLessonId)}" ${l?"":"disabled"}>Avancar para a proxima licao</button>`:`<button type="button" data-lesson-action="complete" ${l?"":"disabled"}>Concluir licao</button>`}
      </div>
    </article>
