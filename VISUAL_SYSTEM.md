...
- A composicao do silex deve preservar impacto sem monopolizar a cena: no Story Block de ferramentas, usar escala cerca de 12% menor que a primeira integracao (`clamp(252px, 27.3vw, 421px)` desktop/tablet e `clamp(211px, 61.6vw, 297px)` mobile), compensando com deslocamento ligeiro para esquerda/baixo para abrir espaco ao texto.
- Depois da escala estar equilibrada, nao continuar a reduzir o silex; melhorar presenca fisica com descida leve (c. 16-18px), rotacao final proxima de `-5deg`, sombra de contacto curta e color grading quente por CSS.
