...
- Story Blocks pre-historicos podem declarar uma imagem especifica em `data/story-blocks.js` atraves de `image.desktop`, `image.mobile`, `image.fit` e `image.position`. `content-service.js` deve preservar estes metadados ao adicionar `lessonId`, `eraKey` e `sectionId`; `lesson-experience.js` troca o sprite pelo `<picture class="prehistory-block-picture">` quando a imagem existe. O CSS usa `data-prehistory-block-image="true"` para dimensoes e offsets proprios em desktop/mobile. Se a imagem remota falhar, o `<img>` fica `hidden` e o fundo local de silex continua a ocupar a caixa para evitar espacos vazios ou icones de imagem partida.
- Ilustracoes PNG ja recortadas devem manter o canal alfa visivel: para `data-prehistory-block-image="true"`, o contentor e o `<picture>` usam `background: transparent`, sem bordas arredondadas nem `overflow: hidden`. Fundos ou sombras de integracao pertencem ao cenario, nunca a uma caixa retangular por tras da ilustracao.
...
- No desktop, os segmentos da barra de progresso pre-historica usam `59.4px × 7.7px`, um aumento de 10% sobre a escala base, e o corpo narrativo usa `font-weight: 525` (mais 5% que o peso 500 de base) para ganhar presenca sem competir com o titulo.
...
- Titulos grandes dos Post Story Blocks nao devem usar vermelho escuro puro, porque ganham peso tipografico excessivo sobre parede iluminada. Usar pigmento ocre/dourado com textura, stroke minimo e sombra quente. A familia tipografica dos titulos deve continuar a dos Story Blocks pre-historicos (`"Amatic SC", cursive`), enquanto o texto corrido, perguntas, opcoes e feedback das pos-licoes devem continuar a usar a escrita manual do corpo narrativo (`"Patrick Hand", cursive`). Os botoes destes ecras devem reutilizar a mesma textura e relevo dos Story Blocks pre-historicos: secundario como pedra cinzenta polida, primario como pedra ocre/dourada, ambos com altura, raio irregular, hover e pressed consistentes.
- O quiz post-story usa escala tipografica mais compacta do que os Story Blocks. Em desktop, o painel de perguntas e as acoes do quiz ativo devem ficar abaixo do centro visual, com offset adicional de `100px` aplicado por `transform: translateY(100px)` no bloco responsivo desktop, para respirar em relacao a barra de progresso e aproveitar melhor a zona iluminada da parede. Evitar `margin-top` para este acerto porque o layout em grid pode absorver a margem sem deslocar visualmente o conjunto. Quando o quiz esta completo, pergunta, opcoes, feedback, resultado e acoes devem ficar em zonas separadas sem sobreposicao. Em mobile, o estado completo deve resumir pergunta + feedback + resultado e esconder a grelha de opcoes respondidas para evitar texto vertical, cartoes demasiado altos ou colisao com o CTA.
...
- No desktop, as acoes da licao e da pos-licao devem partilhar os tokens `--lesson-desktop-actions-left`, `--lesson-desktop-actions-bottom` e `--lesson-desktop-actions-transform`, para ficarem sempre na mesma posicao visual. A barra de progresso das duas etapas deve partilhar `--lesson-desktop-progress-top` e `--lesson-desktop-progress-width`, mantendo tamanho e ancora identicos. Estes tokens so se aplicam acima de `769px`; os overrides mobile continuam independentes.
- As acoes dos Story Blocks pre-historicos devem usar tokens de posicao comuns (`--prehistory-actions-left` e `--prehistory-actions-bottom`) para ocupar espaco vazio da imagem de fundo de forma consistente entre ecras, sem tapar arte rupestre, desenhos de aldeia ou detalhes narrativos. No desktop, a compensacao final e `translate(230px, 130px)` a partir da ancora definida por esses tokens; no mobile prevalecem os overrides de area segura.
- Os artefactos/imagens dos Story Blocks pre-historicos devem obedecer a offsets comuns (`--prehistory-artifact-offset-x/y`) e manter separacao da coluna de leitura; no desktop, a imagem usa `width: clamp(270.75px, 27.075vw, 451.25px)` (menos 5% face a escala anterior) e o deslocamento final soma 127px ao offset horizontal e 24px ao vertical de base. O corpo narrativo recebe `translateX(70px)`, sem deslocar o titulo; no mobile a compensacao equivalente deve ser vertical para baixo. Evitar colocacao demasiado alta ou encostada ao titulo, texto, progresso ou botoes.
