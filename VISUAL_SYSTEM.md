...
- A orientacao final do silex deve ser diagonal e larga, com `rotate(-30deg)` no desktop e `rotate(-24deg)` no mobile. A sombra de contacto vive no `::before` do artefacto como elipse curta, escura e desfocada, separada do sprite e da iluminacao geral para ajuste independente.
