...
- Story Blocks live in `data/story-blocks.js`, are exposed through `getLessonStoryBlocks()` in `content-service.js`, and are rendered by `lesson-experience.js`. Lesson routes move from intro into a fullscreen linear sequence: story blocks, Lesson Summary, Reflection, Historical Debate, Knowledge Check Quiz, Reward, and Next Lesson Teaser. `currentLessonStoryBlockIndex` tracks the active story block and `currentPostStoryStep` tracks the post-story screen. These screens are the lesson structure itself and must not render dashboard chrome, tabs, grids, contextual trails, or scroll-heavy article layouts around them.
- Post-story lesson content is normalised in `lesson-experience.js` through `buildPostLessonContent()`. Per-lesson data can override the generated fallback with `postLesson: { summary, reflection, debate, quiz }`, where `summary` is 3-5 short points, `reflection.question` is an open prompt, `debate.options` contains two balanced positions, and `quiz` contains 3-5 four-option questions with `correctIndex` and `explanation`. Generated post-lesson quiz questions must be anchored to the active lesson title, detail, and summary; do not pull sequential questions from the era-wide quiz bank because that can surface unrelated lesson topics. Reflection text, debate choice, and quiz answers are persisted in `historiaVivaProgress.lessonProgress[lessonId]`.
- Post-story screen titles must share the same vertical starting point across Summary, Reflection, Debate, Quiz, Reward, and Next Lesson Teaser. Keep `.post-story-copy` anchored to the screen top with the shared `--post-story-screen-top` token, and route special cases such as Summary through that same token instead of custom top padding.
- Completed post-story quiz screens add `is-complete` to the `.quiz-screen` article. Keep completion-state layout fixes scoped to that class so the answered final question, feedback, result summary, progress meter, and fixed action buttons do not collide on desktop or compact viewports.
- Lesson progress now counts the full route as `story blocks + summary + reflection + debate + quiz + reward + next teaser`; the lesson is completed only from the next-lesson teaser, where the learner can enter the next lesson, return to the timeline, or finish the current lesson.
- Story Block narrative headings must stay anchored to the active lesson title. Visual mood and artifact selection may respond to block content, but headings must not be inferred from per-block keywords because that can make a correct lesson appear to change title mid-flow.
- Timeline entries in `data/eras/` may remain compact as `[date, event]`; `data/timeline.js` enriches them with category and detail before `content-service.js` turns them into lessons. If a lesson has no explicit `storyBlocks` or long `textExperience`, `content-service.js` must generate Story Blocks anchored to that lesson's event, date, era, sources, and related topics rather than falling back to generic category copy.
- Lesson route rendering is asynchronous because it can wait for background assets and lazy-loaded lesson modules. `renderCategorySections()` must snapshot the active view, era, lesson id, lesson mode, and story-block index before awaiting, then abort if any of them changed. Era changes made while opening a specific lesson must use `renderEra(..., { skipCategoryRender: true })` so an intermediate render cannot replace the selected lesson with the first lesson of the era.
