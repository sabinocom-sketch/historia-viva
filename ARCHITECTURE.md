...
- Post-story lesson content is normalised in `lesson-experience.js` through `buildPostLessonContent()`. Per-lesson data can override the generated fallback with `postLesson: { summary, reflection, debate, quiz }`, where `summary` is 3-5 short points, `reflection.question` is an open prompt, `debate.options` contains two balanced positions, and `quiz` contains 3-5 four-option questions with `correctIndex` and `explanation`. Reflection text, debate choice, and quiz answers are persisted in `historiaVivaProgress.lessonProgress[lessonId]`.
